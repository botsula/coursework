class Remindozaur:
    ...
    def task_creation_web(self, title, notes, due, money):
        '''
        ...
        task = myTaskCreate(self, title, notes, due, money, delta, fin_money, static_money)
        if money == 'M':
            fin_task = {
    ...
    def add_task(self, add_task_adt, rz_list_id):
        '''
    ...
    def finish_task_upload(self, task_adt, rz_list_id):
        '''
        ...
        if task_adt.success:
            self.add_task(myAddTask(task), rz_list_id)
        else:
            result = SERVICE.tasks().update(tasklist=rz_list_id, task=task['id'],
                                            body=task).execute()

    def money_task_up(self, task, rz_list_id, a, b, c, d, new_num):
        '''
    ...
    def fire_task_up(self, task, rz_list_id, a, b, new_num):
        '''
    ...
    def find_all_tasks(self, rz_list_id):
        '''
    ...
    def update_task(self, one_word_name, rz_list_id):
        '''
        ...
        if task:
            ...
            if new_num == 0:
                ...
                self.finish_task_upload(task, rz_list_id)
            else:
                if task.type == 'fire':
                    self.fire_task_up(task, rz_list_id, a, b, new_num)

                if task.type == 'money':
                    if task.success is True:
                        task.money[0] = int(task.money[0]) + int(task.money_today)
                        task.money_today = task.static_money
                        self.money_task_up(task, rz_list_id, a, b, c, d, new_num)
                    else:
                        task.money_today += task.static_money
                        self.money_task_up(task, rz_list_id, a, b, c, d, new_num)
    ...
    def main(self):
        ...
        if choice == '1':
            a = self.task_creation()
            self.add_task(myAddTask(a), id)
            self.main()
        elif choice == '2':
            temp = input('Name of the task: ')
            self.update_task(temp, id)
            self.main()
        elif choice == '3':
            for task_title in self.find_all_tasks(id):
                self.update_task(task_title[0], id)
            self.main()
        elif choice == '4':
            return None
